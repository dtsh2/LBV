...
fullParamSets = cbind(nonVarying,hypercubeadj)
head(fullParamSets)
dim(fullParamSets)
...
BETA = fullParamSets[,22]
MU = fullParamSets[,1]
DELTA = fullParamSets[,2]
ALPHA = fullParamSets[,3]
RHO = fullParamSets[,23]
SIGMA = fullParamSets[,4]
K = fullParamSets[,5]
EPSILON = fullParamSets[,6]
TAU = fullParamSets[,7]
KAPPA = fullParamSets[,8]
S = fullParamSets[,9]
OMEGA = fullParamSets[,10]
PHI = fullParamSets[,11]
SUSJ.0 = fullParamSets[,12]
EIJ.0 = fullParamSets[,13]
ERJ.0 = fullParamSets[,14]
INFJ.0 = fullParamSets[,15]
RECJ.0 = fullParamSets[,16]
SUSA.0 = fullParamSets[,17]
EIA.0 = fullParamSets[,18]
ERA.0 = fullParamSets[,19]
INFA.0 = fullParamSets[,20]
RECA.0 = fullParamSets[,21]
...
library(rgl)
#plot3d(x = paramset[,1], y = paramset[,5], z = X[,2])
...
#
#par(mfrow=c(2,1))
plot(sim$SUSJ[6800:7300], main ="Juvenile",ylim=c(0,200000),col="blue",
...
susjt<-rowSums(sim[,c(1,1:3)])

susat<-rowSums(sim[,c(6:8)])
