...
## ~~~~~~~~~~ LHS SAMPLING ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#############
# NB not rate of aging, # birth pulses

library(lhs)

nspaces=100 ## how many bins/intervals, 100 needed for PRCC

hypercube=randomLHS(n=nspaces, k=11) ## function with N columns
...
nonVarying = matrix(c(
  ...
  #phi= 0.5, # for timing - ...
  SUSJ.0=4000,EIJ.0=1000,ERJ.0=1000,INFJ.0=1000,RECJ.0=10000,SUSA.0=50000, 
...
dimnames(nonVarying)[[2]]=c("epsilon","omega","SUSJ.0","EIJ.0",
                            "ERJ.0","INFJ.0","RECJ.0","SUSA.0",
                            "EIA.0","ERA.0","INFA.0","RECA.0") # naming non-varying columns

fullParamSets = cbind(nonVarying,hypercubeadj) # full parameter set

head(fullParamSets)
dim(fullParamSets)

dimnames(fullParamSets)[[2]]=c("EPSILON","OMEGA","SUSJ.0","EIJ.0",
                               "ERJ.0","INFJ.0","RECJ.0","SUSA.0",
...
BETA = fullParamSets[,13]
MU = fullParamSets[,14]
DELTA = fullParamSets[,15]
ALPHA = fullParamSets[,16]
RHO = fullParamSets[,17]
SIGMA = fullParamSets[,18]
K = fullParamSets[,19]
EPSILON = fullParamSets[,1]
TAU = fullParamSets[,20]
KAPPA = fullParamSets[,21]
S = fullParamSets[,22]
OMEGA = fullParamSets[,2]
PHI = fullParamSets[,23]
SUSJ.0 = fullParamSets[,3]
EIJ.0 = fullParamSets[,4]
ERJ.0 = fullParamSets[,5]
INFJ.0 = fullParamSets[,6]
RECJ.0 = fullParamSets[,7]
SUSA.0 = fullParamSets[,8]
EIA.0 = fullParamSets[,9]
ERA.0 = fullParamSets[,10]
INFA.0 = fullParamSets[,11]
RECA.0 = fullParamSets[,12]
